#Updated 2017-02-15 to serve several tables in prod_ccall. One segment at most can be loaded into one destination table.
...
#This query combines all phone numbers from phones_x_segments according to score and segment share and puts it to phones_to_call
query_pattern = '''
...
while df_segs['updated'].mean() != 1:
    while df_segs[df_segs['updated'] == 0]['raw_ready'].sum() == 0:
        ...
        file_list = os.popen('ls /home/bigdatasys/projects/la_ccall').read()
        df_segs['raw_ready'] = df_segs['segment_nm'].map(lambda s: s + '_ready' in file_list)
    ...
    for s in df_segs.ix[idx,'segment_nm']:
        print 'rm command to execute:  ' + 'rm -f '+ seg_ready_files_path + '/' + s + '_ready'
        sys.stdout.flush()
        os.popen('rm -f '+ seg_ready_files_path + '/' + s + '_ready').read()
