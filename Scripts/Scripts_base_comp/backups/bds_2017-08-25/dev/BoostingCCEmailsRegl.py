...
from
 ...
 left join prod_lookalike.lookalike_coeff_new c on b.urlfr = c.urlfr
where
...
print('Query 11 started at {0}.'.format(datetime.datetime.now()))
cursor.execute (queries_dep.split(';')[-1].replace('#ind', ind).replace('#ymd_start', ymd_start).replace('#ymd_end', ymd_end).replace('#ymd_current', ymd_current))
'''
# In[ ]:
