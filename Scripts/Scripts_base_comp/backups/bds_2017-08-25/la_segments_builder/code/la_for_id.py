...
def visits_ready():
    res = False
    cursor.execute('select * from prod_odd.visit_feature where ymd = "#ymd" and load_src = "LI.02" limit 100'.replace('#ymd', ymd))
    if len(cursor.fetchall()) == 100:
        res = True
    return res

while not visits_ready():
    print '{0} Visits not ready, falling asleep for 20 minutes'.format(datetime.datetime.now())
    sys.stdout.flush()
    sleep(1200)
...
select
 ...
 ,max("ymd_dt") as ymd
from
 prod_odd.visit_feature a
 inner join prod_lookalike.urlfr_coeff b on a.url_fragment = b.urlfr 
 left join prod_lookalike.threshold t on t.segment_nm = b.segment_nm
where 
 a.ymd in ("ymd_dt")
 and a.load_src = "LI.02"
 and b.score >= nvl(t.threshold,-999999)
 and b.segment_nm in ('dmp_relevant_full_apps_2016-10-15_2016-11-15', 'sme_apps_v0.2')
group by
  a.id
 ,b.segment_nm
