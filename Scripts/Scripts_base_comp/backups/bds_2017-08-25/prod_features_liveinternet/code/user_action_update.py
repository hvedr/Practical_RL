# updated 2017-05-03 by k.p.osminin
...
cursor.execute('select coalesce(max(ymd), "2017-04-05") from prod_features_liveinternet.user_action where CAST(ymd AS Date) IS NOT NULL')
...
ymd_end = str(datetime.date.today()-datetime.timedelta(days=4))
...
queries = '''
    ...
    select
		...
		,max(0) as revisited
        ,max(c.decision_approve_flg) as decision_approve_flg
        ,if((regexp_extract(lower(a.product_name), 'all.?airlines',0) <> '') or (regexp_extract(lower(a.lp), 'credit.*all?airlines',0) <> ''),'AA','Platinum' ) as product
        ,count(*) as apps
    from
    ...
    where 
		...
		and
         (
          (
	 		(
			 (
			  (regexp_extract(lower(a.product_name), 'platinum',0) <> '') 
                          and a.lp not in ('/credit/formliteagent/loan/', '/cards/credit-cards/cobrands/auramall/agent/form/short/', '/mgm/form/')
			 )
			 or b.lp is not null
			)
        ...
        and a.agent_code is null
          ...
          (
           (
            (regexp_extract(lower(a.product_name), 'all.?airlines', 0) <> '') 
    ...
    group by 
        a.ymd
		,a.wuid
        ,if((regexp_extract(lower(a.product_name), 'all.?airlines', 0) <> '') or (regexp_extract(lower(a.lp), 'credit.*all?airlines', 0) <> ''),'AA','Platinum' )
;
...
insert into prod_features_liveinternet.cc_wuid_#ind
    select
    	...
    	,max(1) as completed_flag
    	,max(1) as revisited
        ,max(s.decision_approve_flg) as decision_approve_flg
...
for q in query_list: cursor.execute(q) # print(q + ';') # 
