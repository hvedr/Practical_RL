...
print('{}. Getting visits later than {} and main segment updated.\n'.format(datetime.datetime.now(),ymd_loaded))
sys.stdout.flush()
...
while (ymd_calculated >= ymd_loaded) | (ymd_calculated >= ymd_main_seg):
    ...
    if cnt > 70:
        ...
        sys.stdout.flush()
        exit(1)
    print('Sleeping for 1000 sec. ymd_calculated: {}. ymd_loaded: {}. ymd_main_seg: {}.'.format(ymd_calculated,ymd_loaded,ymd_main_seg))
    sys.stdout.flush()
    time.sleep(1000)
    ymd_loaded = get_last_visits_date(cursor)
    ymd_main_seg   = get_last_main_seg_calc_date(cursor)
