...
cursor.execute('select destination, segment_nm from prod_lookalike.segs_to_reload')
df_reload = pd.DataFrame([[e[0],e[1],'',0] for e in cursor.fetchall()], columns = ['destination', 'segment_nm', 'ymd_loaded','updated'])
...
for tab_name in df_reload ['destination'].unique():
    cursor.execute('''select nvl(max(ymd),'2017-01-01') from #tab_name'''.replace('#tab_name',tab_name))
    df_reload.loc[df_reload['destination'] == tab_name,'ymd_loaded'] = cursor.fetchone()[0]
...
query_pattern_w_filter_not_used = '''
with csymd as 
  (
  ...
  from
    (
     select
       a.ymd
      ,a.phone_num
      ,b.coef * (row_number() over (partition by a.ymd, a.segment_nm order by a.score desc)) as rnk
      ,a.segment_nm
      ,a.score
     ...
     from
      ...
      left join big_data_science.segment_one_shot_unique d on d.phone_num = a.phone_num
     where a.ymd > '#ymd_loaded'
       and c.segment_nm = 'cred_score_1'
       and nvl(c.score,1) >= b.cred_threshold
       and d.phone_num is Null   
    ) d
...
select
  ...
  ,ymd  
from
  big_data_science.ccall_segments
where
  ymd > '#ymd_loaded'
  and segment_nm in (#segs)
...
while df_segs['updated'].mean() != 1:
    ...
    for tab_name, dest in df_reload.ix[idx].groupby('destination'):
        segs = "'" + "','".join(df_reload[df_reload['destination'] == tab_name]['segment_nm'].values) + "'"
        ymd_loaded = dest['ymd_loaded'].iloc[0]
        q = reload_query_pattern.replace('#segs',segs).replace('#ymd_loaded',ymd_loaded).replace('#tab_name',tab_name)
        print(q)
        sys.stdout.flush()
        cursor.execute(q)
