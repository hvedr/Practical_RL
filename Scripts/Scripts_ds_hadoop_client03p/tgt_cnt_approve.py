...
action_type_to_load = ['tinkoff_platinum_approved_application']
...
for d in range(0,31,1):
    for at in action_type_to_load:
        ymd_to_process = datetime.date(2016,11,07)  + datetime.timedelta(days=d)
        ymd0      = ymd_to_process.strftime('%Y-%m-%d')
        ymd_start = (ymd_to_process).strftime('%Y-%m-%d')
        ymd_end   = (ymd_to_process).strftime('%Y-%m-%d')
        N = ''
