...
ymd_start = sys.argv[1]
ymd_end = sys.argv[2]

print('Calc user_action_new for {} - {}'.format(ymd_start,ymd_end))
...
queries = '''
    ...
    select
		...
		,max(0) as revisited
        ,case when max(c.decision_approve_flg) in ('Y',1) then 1 else 0 end as decision_approve_flg
        ,case when a.product_name in ('cc_allairlines','Tinkoff All Airlines') 
    ...
    where 
		...
		and
         (
          (
	 		(
			 (
			  a.product_name in ('cc_platinum','Tinkoff Platinum') and a.lp not in ('/credit/formliteagent/loan/', '/cards/credit-cards/cobrands/auramall/agent/form/short/', '/mgm/form/')
			 )
			 or b.lp is not null
			)
        ...
        and a.agent_code is null
          ...
          (
           (
            a.product_name in ('cc_allairlines','Tinkoff All Airlines') or a.lp in ('/cards/credit-cards/all-airlines/','/credit/cards/allairlines/','/credit/form/aa/allairlinesdiscounts/','/credit/form/mgmallairlines/')
            )
         and lower(a.lp) not like '%agent%'
         and a.agent_code is null
...
insert into prod_features_liveinternet.cc_wuid_#ind
    select
    	...
    	,max(1) as completed_flag
    	,max(1) as revisited
        ,case when max(s.decision_approve_flg) in ('Y',1) then 1 else 0 end as decision_approve_flg
...
drop table if exists prod_features_liveinternet.cc_wuid_li_#ind;
    ...
    insert overwrite
		table prod_features_liveinternet.user_action_new partition(action_type, ymd)
	select
		li_id as id
		,dt_created as time
